...
class ImmunoPhenoDB_Connect:
    ...
    def merge_quadrants(self,
        ...
        """
        Iteratively finds partitions with fewer than min_points and merges them
        with an adjacent neighbor until all partitions meet the minimum size.
        ...
        Returns:
            pd.DataFrame: A new, updated boundary_table with small partitions merged.
        """
        
        # Work on a copy to avoid modifying the original table in place
        merged_table = boundary_table.copy()

        while True:
            # 1. Identify all boxes smaller than the threshold
            small_boxes = merged_table[merged_table['count'] < min_points]
            
            # If there are none, return nothing
            if small_boxes.empty:
                break
                
            # Get the first small box to process
            # We process one at a time because merging changes the table
            small_box = small_boxes.iloc[0]
            
            # 2. Find a neighbor for this small box
            neighbor_box = None
            for index, potential_neighbor in merged_table.iterrows():
                # Skip itself
                if small_box['Box ID'] == potential_neighbor['Box ID']:
                    continue
                
                # Check for shared edges (adjacency)
                # A shared vertical edge means x_max of one is x_min of the other, with some y-overlap
                share_vertical_edge = (abs(small_box['x_max'] - potential_neighbor['x_min']) < 1e-9 or \
                                    abs(small_box['x_min'] - potential_neighbor['x_max']) < 1e-9)
                y_overlap = (small_box['y_min'] < potential_neighbor['y_max'] and \
                            small_box['y_max'] > potential_neighbor['y_min'])
                
                # A shared horizontal edge means y_max of one is y_min of the other, with some x-overlap
                share_horizontal_edge = (abs(small_box['y_max'] - potential_neighbor['y_min']) < 1e-9 or \
                                        abs(small_box['y_min'] - potential_neighbor['y_max']) < 1e-9)
                x_overlap = (small_box['x_min'] < potential_neighbor['x_max'] and \
                            small_box['x_max'] > potential_neighbor['x_min'])

                if (share_vertical_edge and y_overlap) or (share_horizontal_edge and x_overlap):
                    neighbor_box = potential_neighbor
                    break # Found a neighbor, stop searching

            if neighbor_box is None:
                # This can happen if a small box is completely isolated
                print(f"Warning: Could not find a neighbor for Box ID {small_box['Box ID']}. It will not be merged.")
                # To prevent an infinite loop, we'll mark this box as 'processed' by temporarily giving it a large count
                merged_table.loc[merged_table['Box ID'] == small_box['Box ID'], 'count'] = min_points
                continue

            # 3. Perform the merge
            print(f"Merging Box {int(small_box['Box ID'])} (count={small_box['count']}) with Box {int(neighbor_box['Box ID'])} (count={neighbor_box['count']})")
            
            new_x_min = min(small_box['x_min'], neighbor_box['x_min'])
            new_y_min = min(small_box['y_min'], neighbor_box['y_min'])
            new_x_max = max(small_box['x_max'], neighbor_box['x_max'])
            new_y_max = max(small_box['y_max'], neighbor_box['y_max'])
            new_count = small_box['count'] + neighbor_box['count']
            
            new_box_data = {
                'Box ID': -1, # Temporary ID
                'count': new_count,
                'x_min': new_x_min,
                'x_max': new_x_max,
                'y_min': new_y_min,
            ...
            # 4. Update the table
            # Remove the two old boxes
            merged_table = merged_table.drop(index=[small_box.name, neighbor_box.name])
            
            # Add the new merged box
            merged_table = pd.concat([merged_table, pd.DataFrame([new_box_data])], ignore_index=True)

        # 5. Re-index the Box IDs to be clean and sequential
        merged_table = merged_table.reset_index(drop=True)
        merged_table['Box ID'] = merged_table.index
    ...
    def run_stvea(self,
        ...
        # 10/9/2025 - Skip idCL conversion to celltype if using own provided reference
        if IPD_reference:
            labels_df["celltype"] = labels_df["labels"] # Use same as "labels" field
        else:
            convert_idCL = {
    ...
    def optimal_antibody_panel(self,
        ...
        if plot_decision_tree:
            ...
            # Create an HTML-like label for the legend table.
            header = '<TR><TD COLSPAN="2" ALIGN="LEFT"><B>Value Legend</B></TD></TR>'
            rows = []
            for i, name in enumerate(cart.tree.classes_.astype(str)):
                # Escape any special HTML characters in the class name for safety
                safe_name = html.escape(name)
                rows.append(f'<TR><TD ALIGN="LEFT" VALIGN="TOP">{i}:</TD><TD ALIGN="LEFT">{safe_name}</TD></TR>')
            
            table_content = "".join([header] + rows)
            html_label = f'<<TABLE BORDER="1" CELLBORDER="0" CELLSPACING="0" CELLPADDING="4">{table_content}</TABLE>>'

            # Set the graph's label to the HTML table we just created.
            graph.set('label', html_label)
            # Place the legend at the bottom of the image ('b' for bottom, 't' for top).
            graph.set('labelloc', 't')
            # Set a font size for the legend.
            graph.set('fontsize', '14')

            # Write the final PNG with the tree and the non-overlapping legend.
